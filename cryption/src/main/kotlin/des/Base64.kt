...
object Base64 {
    ...
    fun encode(data: ByteArray): String {
        ...
        while (i <= end) {
            val d =
                data[i].toInt() and 0x0ff shl 16 or (data[i + 1].toInt() and 0x0ff shl 8) or (data[i + 2].toInt() and 0x0ff)
    ...
    private fun decode(s: String, os: OutputStream) {
        ...
        while (true) {
            ...
            val tri = (decode(s[i]) shl 18) + (decode(s[i + 1]) shl 12) + (decode(s[i + 2]) shl 6) + decode(s[i + 3])
    ...
    fun main(args: Array<String>) {
        val content = "aaaa"
        println("加密之前：$content")

        // 加密
        val jiami = Base64.encode(content.toByteArray(charset("utf-8")))
        println("加密后的内容：$jiami")

        // 解密
        val jiemi = String(Base64.decode(jiami))
        println("解密后的内容：$jiemi")
    }
}
